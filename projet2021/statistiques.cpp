    ...
    ui(new Ui::Statistiques)
{
    ...
    nuclear->moveAbove(fossil);
      ...
      query.prepare("SELECT DISTINCT idinterview FROM interview");
      query.exec();
      ...
      while(query.next())
      {
       ...
       query1.prepare("SELECT idinterview FROM interview where idinterview = '"+query.value(0).toString()+"' ");
       query1.exec();
       while (query1.next()) {
         fossilData.append(query1.value(0).toDouble());
       }
       query2.prepare("SELECT idinterview FROM interview where idinterview = '"+query.value(0).toString()+"' ");
       query2.exec();
       while (query2.next()) {
    ...
    //  labels << "5000" << "10000" << "15000" ;
      ...
      ui->widget->yAxis->setLabel("Sport         Art           Politique         ");
